    ...
    namespace vp {
        ...
        void static_stabling::PerformAlgorithm(const cv::Mat &src, cv::Mat &dst) {
            ...
            if (!init_points.empty()) {
                ...
                init_points[0].erase(init_points[0].begin() + 37);
                init_points[1].erase((init_points[1].begin()));
            }
            ...
            for (int i = 0; i < area.size(); ++i) {
                if (index_out[i] != -1) {
                    ...
                    cv::putText(dst, std::to_string(index_out[i]), center[i], cv::FONT_ITALIC, 0.8,
                                cv::Scalar(255, 0, 0), 2);
                } else {
            ...
            cv::cvtColor(bgmask, canvas(cv::Rect(show_roi.width, 0, show_roi.width, show_roi.height)),
                         cv::COLOR_GRAY2BGR);
        ...
        void static_stabling::init(const cv::Mat &src) {
            ...
            roi2f = roi = cv::Rect(cv::Point(0, 310), cv::Point(src.cols, 903));
            show_roi = cv::Rect(cv::Point(300, 200), max_rect.br() - cv::Point(300, 200));
            init_flag = true;
            homograph_matrix.at<double>(0, 0) = homograph_matrix.at<double>(1, 1) = homograph_matrix.at<double>(2,
                                                                                                                2) = 1;
            std::cout << "INIT DONE\n";
        }

        void static_stabling::MyRefine(cv::Mat &mask, int radius, std::vector<cv::Rect2f> &bboxs,
                                       std::vector<cv::Point2f> &mass_center, std::vector<float> &area) {
            mass_center.clear();
            area.clear();
            mask = (mask > 130);
        ...
        void static_stabling::MyHoughLine(const std::vector<cv::Point2f> &points, const int &threshold,
            ...
            for (int i = 0; i < count_map.size(); i++) {
                ...
                for (int j = 0; j < item.size(); j++) {
                    if (item[j] > threshold) {
                        pair_res.emplace_back(
                                std::pair<int, cv::Vec2f>(item[j], cv::Vec2f(i * 180 / (float) theta_num, j * den_q)));
                    }
                }
            }
            std::sort(pair_res.begin(), pair_res.end(),
                      [](const std::pair<int, cv::Vec2f> &v1, const std::pair<int, cv::Vec2f> &v2) -> bool {
        ...
        void static_stabling::GetPointsPair(std::vector<std::vector<cv::Point2f> > &points, const cv::Mat &gray,
                                            const cv::Rect2f &roi, const cv::Mat &mask) {
            points.clear();
            ...
            for (int i = 0; i < 2 && i < lines.size(); i++) {
                ...
                cv::line(line_mask, cv::Point2f(x1, y1) + roi2f.tl(), cv::Point2f(x2, y2) + roi2f.tl(),
                         cv::Scalar((i + 1) * 122), 13);
            }
            ...
            std::sort(points[0].begin(), points[0].end(),
                      [](const cv::Point2f &p1, const cv::Point2f &p2) -> bool { return p1.x > p2.x; });
        ...
        void static_stabling::Match(const std::vector<std::vector<cv::Point2f> > &points) {
            ...
            cv::Mat out_mask;
            homograph_matrix = cv::findHomography(matched_points[0], matched_points[1], cv::RANSAC, 3, out_mask);
        }
