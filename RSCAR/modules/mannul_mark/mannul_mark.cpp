...
namespace rs{
    namespace vp{
        ...
        main_program_run_flag(true), key(-1), mark_state(INIT_FRAME), object_index(0), object_type(PERSON){
            ReadConfig();
            ReadData();
                            ...
                            switch (key) {
                                ...
                                case 's': {
                                    ...
                                    for (auto iter = frame_data_list.begin(); iter != frame_data_list.end();) {
                                        if (common::CalculateRectOverlapRatio(iter->track_data.bbox, tmp, common::AND_OR) > 0.0)
