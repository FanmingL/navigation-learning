...
namespace rs {
    namespace vp {
        preprocess::preprocess(const std::string &name) : rs(name) {
            ...
            if (if_write_video()) {
                std::remove((char *) common::GetAbsolutePath(preprocess_config.video_output_path()).c_str());
                video_writer.open(common::GetAbsolutePath(preprocess_config.video_output_path()),
                                  CV_FOURCC('D', 'I', 'V', 'X'), preprocess_config.fps(),
                                  cv::Size(preprocess_config.width_final(), preprocess_config.height_final()));
            }
            video_capture.open(common::GetAbsolutePath(preprocess_config.video_input_path()));
