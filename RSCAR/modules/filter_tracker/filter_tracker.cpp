...
namespace rs {
    namespace vp {
        ...
        void tracker_filter::Run() {
            ...
            for (auto &item : out_video_data.frame()) {
                ...
                for (auto &item2 : item.object()) {
                    ...
                    if (bicycle_count.count(item2.car_index()) &&
                        bicycle_count[item2.car_index()] > filter_tracker_config.shift_type_count()) {
                        iter2->set_name("bicycle");
                    } else if (motor_count.count(item2.car_index()) &&
                               motor_count[item2.car_index()] > filter_tracker_config.shift_type_count()) {
        ...
        void tracker_filter::ReadConfig() {
            common::ReadProtoFromTextFile("modules/filter_tracker/config/filter_tracker.prototxt",
        ...
        void tracker_filter::DrawRect(cv::Mat &src, const DetectFrame &data) {
            for (auto &item : data.object()) {
                ...
                cv::putText(src, std::to_string(item.car_index()), rect_tmp.tl(), cv::FONT_ITALIC, 0.8,
                            cv::Scalar(0, 0, 255), 2);
            }
        }
            ...
            for (auto &item: in_data.object()) {
                ...
                for (auto &item2 : raw_data.object()) {
                    ...
                    if (item2.name() == "motorcycle") {
                        if (common::CalculateRectOverlapRatio(rect2, rect_temp, common::AND_MIN) >
                            filter_tracker_config.shift_overlap()) {
                ...
                gap_it:;
                auto *iter = out_data->add_object();
                if (bicycle_count.count(item.car_index()) &&
                    bicycle_count[item.car_index()] > filter_tracker_config.shift_type_count()) {
                    iter->set_name("bicycle");
                } else if (motor_count.count(item.car_index()) &&
                           motor_count[item.car_index()] > filter_tracker_config.shift_type_count()) {
