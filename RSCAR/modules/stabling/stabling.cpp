...
namespace rs {
    namespace vp {
        ...
        descriptor(cv::xfeatures2d::LATCH::create()),
          bfMatcher( cv::NORM_HAMMING ),
            ...
            if (stabling_config.if_write_video()) {
                ...
                video_writer.open(common::GetAbsolutePath(stabling_config.out_video_path()),
        ...
        void stabling::CalculateHomograph(const cv::Mat &src, const DetectFrame &detect_frame, cv::Mat &dst) {
            ...
            cv::resize(mask, mask, cv::Size(stabling_config.target_width(), stabling_config.target_height()), 0, 0,
                       cv::INTER_NEAREST);
            ...
            cv::Mat out_mask;
            last_homograph = last_homograph *
                             cv::findHomography(now_points, last_points, cv::RANSAC, stabling_config.ransac_threshold(),
                                                out_mask);
            std::cout << last_homograph;
            cv::Mat canvas;
            cv::drawMatches(last_frame, last_key_points, gray, key_points, good_matches, canvas, cv::Scalar(0, 0, 255),
                            cv::Scalar(255, 0, 0), out_mask);
