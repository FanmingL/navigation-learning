...
namespace rs{
    ...
    for (int frame_index = 0; frame_index < video_data.frame_size(); frame_index++)
    {
        ...
        if (frame_index % 100 == 99) {
            std::ofstream of(common::get_absolute_path("data/debug.txt"), std::ios::trunc);
        ...
        for (; j < frame.object().size(); j++)
        {
            ...
            filter_map[item.object_index()] = std::make_shared<common::mean_filter<cv::Point2f> >(cv::Point2f(item.world_position_x(),
        ...
        for (auto &item : position_after_filter)
        {
            ...
            frame_mask(rect_fill_color_tmp & image_max_rect) = cv::Scalar(string_image_map[frame.object(car_index).name()]);
        }
        ...
        for (auto &item : position_after_filter)
        {
            ...
            iter->set_angle_in_world((float)(head_angle / CV_PI * 180));
                                ...
                                if (val_here != 255 )
                                {
                                    ...
                                    switch (val_here){
                                        case (int)common::CANNOT_GO:
                                            surround_type_image[angle_i] = OBSTACLE;break_flag = true;
                                            break;
                                        case (int)common::CAR:
                                            surround_type_image[angle_i] = CAR;break_flag = true;
                                            break;
                                        case (int)common::PERSON:
                                            surround_type_image[angle_i] = PERSON;break_flag = true;
                                            break;
                                        case (int)common::BICYCLE:
                                            surround_type_image[angle_i] = BICYCLE;break_flag = true;
                                            break;
                                        case(int)common::MOTORBIKE:
                                            surround_type_image[angle_i] = MOTORBIKE;break_flag = true;
                                            break;
                ...
                for (int angle_i = 0; angle_i  < 360 ;angle_i++)
                {
                    ...
                    common::CalculateTransform(homograph_matrix, point_image_obstacle[angle_i], res_world_point_tmp);
        ...
        void information::ReadMask() {
            mask = cv::imread(common::get_absolute_path(information_config.mask_path()), cv::IMREAD_GRAYSCALE);
        }
